...
export default function EditPart({ fid, closeEvent }) {
  ...
  useEffect(() => {
    setPartData({
      ...
      partpreprocessing:
        preprocessingOptions.find((option) => option.value === fid.preprocess)
          ?.label || '',
      partmemo: fid.memo || '',
    });
      ...
      <Grid container spacing={2}>
        ...
        <Grid item xs={3}>
          ...
          <TextField
            error={weightError}
            helperText={
              weightError ? '0 이상의 숫자만 입력할 수 있습니다.' : ' '
            }
            id="outlined-basic"
            label="Part Weight(g)"
            variant="outlined"
