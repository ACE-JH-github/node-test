...
import EditPartMaterial from './EditPartMaterial';
import { db } from '../../api/firebase';
import AddPartMaterials from './AddPartMaterial';
import './CSS/sweetalert2.css';
import PartSubstanceList from './PartSubstanceList';
...
export default function PartMaterialList({ closeEvent, initialPartID }) {
  ...
  const getMaterials = () => {
    const materialCollectionRef = collection(db, 'parts', currentPartID, 'materials');
  ...
  const deleteApi = async (id) => {
    const userDoc = doc(db, 'parts', currentPartID, 'materials', id);
  ...
  const editMaterialData = (id, partmaterialgroup, partmaterialname, partrecycledcontent, partrecycledtype, partmaterialmass) => {
    const data = {
      id: id,
  ...
  const viewSubstancesData = (id) => {
    ...
    setCurrentPartID(currentPartID); // 추가된 부분
    ...
    <>
      ...
      <div>
        <Modal open={open} aria-labelledby="modal-modal-title" aria-describedby="modal-modal-description">
          <Box sx={style}>
            <AddPartMaterials closeEvent={handleClose} currentPartID={currentPartID} />
          </Box>
        </Modal>
